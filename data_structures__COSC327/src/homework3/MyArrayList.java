...
public class MyArrayList<T> implements MyList<T>, Cloneable, Serializable
{
    ...
    //Return: A clone of MyArrayList
    //Note:   listArr is passed with references to objects
    public Object clone()
    {
        MyArrayList<T> copy = null;
        try
        {
            copy = (MyArrayList<T>)super.clone();
        }
    ...
    //Post: 	item added to this collection if item does not exist or if it exists but the
    // 	collection permits duplicates
    //Return: 	True if the operation inserts a new element in this collection; false if this
    //	collection already contains the specified element and does not permit duplicates.
